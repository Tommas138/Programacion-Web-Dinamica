...
$data = data_submitted();
$objAuto = new C_Auto();
$buscar = $objAuto->buscar($data);
?>

<div class="container col-md-5 " style="margin:30px;height:150vh;">
    <?php
    if (isset($buscar)) {
        echo '<table class="table table-primary table-striped table-bordered " > <th> <h3>Patente:</h3> </th> <td>'.$buscar[0]->getPatente().'</td></tr>
                    <tr> <th > <h3>Marca:</h3> </th> <td>'.$buscar[0]->getMarca().'</td> </tr>
                    <tr> <th> <h3>Modelo:</h3> </th> <td>'.$buscar[0]->getModelo().'</td> </tr>
                    <tr> <th> <h3>Nombre del duenio:</h3>  </th> <td>'.$buscar[0]->getObjDuenio()->getNombre(). " " . $buscar[0]->getObjDuenio()->getApellido() .'</td> </tr>
                    <tr> <th> <h3>Dni del duenio:</h3>  </th> <td>'.$buscar[0]->getObjDuenio()->getNroDni().'</td> </tr>
                </table>';
    } else {
